const state = {
    ...
    <div class='col-md-6 col-lg-4 mt-3' id=${id} key=${id}>
      <div class='card shadow-sm task__card'>
        <div class='card-header d-flex gap-2 justify-content-end task__card__header'>
          <button type='button' class='btn btn-outline-dark mr-2' name=${id} onclick="editTask.apply(this, arguments)">
        ...
        <div class='card-body'>
         ...
         <p id='displayDateTime'></p>
          ...
          <div class='tags text-white d-flex flex-wrap'>
            <span class='badge bg-warning m-1 '>${type}</span>
          </div>
        </div>
  ...
  <strong id="strong"class="text-sm text-muted">Created on ${date.toDateString()}</strong>
  ...
  const deleteTask = (e) => {
    ...
    // get a div which u want to delete on clicling delete btn
      if (type === "BUTTON") {
        // go 4 times back from button  to access  card and remove it
    ...
    ); // go 3 times inside to access  card
  };
  ...
  // the open task attribute which we hVE DELETed for save changes btn we will add it back
      // the changes which we have made in ui will undo it
